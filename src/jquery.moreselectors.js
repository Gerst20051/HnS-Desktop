/*
  ...
  Compatibility update for 1.3.x by Karl Swedberg.
  ...
  v.1.3.2   - 24-Feb-2009 Fully updated for jQuery 1.3.2. Removed experimental $.filter function. 
*/
...
jQuery.extend(jQuery.expr[':'], {
  ...
  // Text check:
  Contains  : function(a,i,m) {return (a.innerText||a.innerHTML).toUpperCase().indexOf(m[3].toUpperCase())>=0;},  // Same as :contains() but ignores case.
  ...
  text    : function(a,i,m) {return !m[3] ? a.type=='text' || a.nodeName=='TEXTAREA' : ( jQuery(a).text().toLowerCase()==m[3].toLowerCase() || (a.value && a.value.toLowerCase()==m[3].toLowerCase()) );},


  // Form attributes:
  hover   : function(a,i,m) {return a==document.hoverElement;},   // hoverElement is custom property maintained by dedicated document.mouseover event (see below).
  focus   : function(a,i,m) {return a==document.activeElement;},    // activeElement is natively available in IE. Is custom property in other browsers, maintained by dedicated blur/focus events.
  blur    : function(a,i,m) {return a==document.lastActiveElement;},  // lastActiveElement is custom property maintained by dedicated blur/focus events.
  modified  : function(a,i,m){
    return (a.nodeName=='INPUT' && (a.type=='checkbox' || a.type=='radio') && a.defaultChecked != a.checked) 
      || (a.nodeName=='INPUT' && a.defaultValue != a.value && {'text':true,'hidden':true,'file':true,'password':true}[a.type]) 
      || (a.nodeName=='TEXTAREA' && a.defaultValue != a.value) 
      || ( a.nodeName=='SELECT' &&  jQuery(a).is('[OPTION:option-mod]') );
  },
  unchecked : function(a,i,m) {return a.nodeName=='INPUT' && !a.checked;},
  selected  : function(a,i,m) {return (a.nodeName=='SELECT' && jQuery('OPTION[selected]',a).is()) || (a.nodeName=='OPTION' && a.selected);},
  "option-sel": function(a,i,m) {return (a.nodeName=='OPTION' && a.selected) || (a.nodeName=='INPUT' && a.checked && (a.type=='checkbox' || a.type=='radio'));},
  "option-def": function(a,i,m) {return (a.nodeName=='OPTION' && a.defaultSelected) || (a.nodeName=='INPUT' && (a.type=='checkbox' || a.type=='radio') && a.defaultChecked);},
  "option-mod": function(a,i,m) {return (a.nodeName=='OPTION' && a.defaultSelected != a.selected) || (a.nodeName=='INPUT' && (a.type=='checkbox' || a.type=='radio') && a.defaultChecked != a.checked);},
  ...
  textarea  : function(a,i,m) {return a.nodeName=='TEXTAREA';},
  select    : function(a,i,m) {return a.nodeName=='SELECT';},
  multiple  : function(a,i,m) {return a.nodeName=='SELECT' && a.multiple;},
  option    : function(a,i,m) {return (a.nodeName=='OPTION') || ( a.nodeName=='INPUT' && (a.type=='checkbox' || a.type=='radio') );},
  // Match elefunction(a,i,m) {return ent name with a regular expression: (eg: $(xml).find( "//customer/*:name('addressline[0-5]')" )
  nodename  : function(a,i,m) {return (new RegExp(m[3])).test(a.nodeName);},
  ...
  siblings  : function(a,i,m) {return jQuery(a).siblings(m[3]).length>0;},
  parents   : function(a,i,m) {return jQuery(a).parents(m[3]).length>0;},


  // Type and Child checks:
  "only-of-type"  :     function(a,i,m) {return 1==jQuery(r).filter( a.nodeName!='INPUT' ? a.nodeName : a.nodeName+'[type='+a.type+']').length;},
  "first-of-type" :     function(a,i,m) {return a==jQuery(r).filter( a.nodeName!='INPUT' ? a.nodeName : a.nodeName+'[type='+a.type+']')[0];},
  "nth-of-type" :       function(a,i,m) {return a==jQuery(r).filter( a.nodeName!='INPUT' ? a.nodeName : a.nodeName+'[type='+a.type+']')[m[3]];},
  "last-of-type"  :     function(a,i,m) {return a==jQuery(r).filter((a.nodeName!='INPUT' ? a.nodeName : a.nodeName+'[type='+a.type+']') +':last')[0];},
  // Note: These use the enhanced siblings() method which accepts extra param to keep current element (self) in the results:
  "only-child-of-type"  : function(a,i,m) {return 1==jQuery(a).siblings(  a.nodeName!='INPUT' ? a.nodeName : a.nodeName+'[type='+a.type+']', true).length;},
  "first-child-of-type" : function(a,i,m) {return a==jQuery(a).siblings(  a.nodeName!='INPUT' ? a.nodeName : a.nodeName+'[type='+a.type+']', true)[0];},
  "nth-child-of-type"   : function(a,i,m) {return a==jQuery(a).siblings(  a.nodeName!='INPUT' ? a.nodeName : a.nodeName+'[type='+a.type+']', true)[m[3]];},
  "last-child-of-type"  : function(a,i,m) {return a==jQuery(a).siblings( (a.nodeName!='INPUT' ? a.nodeName : a.nodeName+'[type='+a.type+']', true) +':last')[0];},
  "nth-last-child"      : function(a,i,m) {return jQuery.sibling(a,jQuery.sibling(a).length-1-m[3]).cur;},
  ...
  "in-view"   : function(a,i,m) {return a.scrollTop==0 && jQuery(a).parent().is(':in-view');},
  // Use our custom $jqColors object (defined below) to compare colours even when they're in different formats:
  color     : function(a,i,m) {return $jqColors.match( m[3], jQuery(a).css('color') );},
  "bg-color"    : function(a,i,m) {return $jqColors.match( m[3], jQuery(a).css('backgroundColor') );},

  "bg-url"    : function(a,i,m) {return a.currentStyle.backgroundImage.toLowerCase().indexOf(m[3].toLowerCase()) != -1;}, // Work in progress!
  ...
  target      : function(a,i,m) {return a.id && jQuery(document.getElementsByTagName('A')).is('[href*=#' + a.id + ']');},
  ...
  colIndex    : function(a,i,m) {return (a.nodeName=='TD' || a.nodeName=='TH') && ( m[3] == jQuery(a).siblings('TH,TD').lt( (parseInt(jQuery(a).attr('cellIndex'),10) || 0) ).sum('colSpan',1) );},
  ...
  max     : function(a,i,m) {return jQuery(a).css(m[3]) != undefined ? jQuery(a).css(m[3]) == Math.max.apply({}, jQuery.map( r, function(el){return jQuery(el).css(m[3]);} )) : jQuery(a).attr(m[3]) == Math.max.apply({}, jQuery.map( r, function(el){return jQuery(el).attr(m[3]);} ));},
  min     : function(a,i,m) {return jQuery(a).css(m[3]) != undefined ? jQuery(a).css(m[3]) == Math.min.apply({}, jQuery.map( r, function(el){return jQuery(el).css(m[3]);} )) : jQuery(a).attr(m[3]) == Math.min.apply({}, jQuery.map( r, function(el){return jQuery(el).attr(m[3]);} ));}
  ...
  jQuery.fn.colIndex = function(){
    var $cell = this.filter("TH,TD").eq(0),           // Get first cell in current selection.
      cellIdx = parseInt($cell.attr("cellIndex"),10) || 0;
  ...
  jQuery.fn.colCells = function(colIdx){
    ...
    var cells = []; colIdx = parseInt(colIdx,10) || null;
